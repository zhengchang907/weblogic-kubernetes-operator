...
echo "called introspect.sh" >> domainAction.log
...
if [ -z "$wls_domain_uid" ]
then
    echo "Usage: introspect.sh --domain_uid=<domain uid> [--kubernetes_master=https://${KUBERNETES_SERVICE_HOST}:${KUBERNETES_SERVICE_PORT}] [--access_token=<access_token>] [--wls_domain_namespace=default] [--operator_namespace=weblogic-operator] [--operator_service_name=weblogic-operator]"
    echo "  where"
...
echo "wls_domain_uid: $wls_domain_uid" >> domainAction.log
echo "wls_domain_namespace: $wls_domain_namespace" >> domainAction.log
echo "operator_service_name: $operator_service_name" >> domainAction.log
echo "operator_service_account: $operator_service_account" >> domainAction.log
echo "operator_namespace: $operator_namespace" >> domainAction.log
...
if [ $? -ne 0 ]
  then
    echo "Failed to retrieve status of $operator_service_name in name space: $operator_namespace" >> domainAction.log
    echo "STATUS: $STATUS" >> domainAction.log
    exit 1
fi
...
echo "port: $port" >> domainAction.log
...
if [ $? -ne 0 ]
  then
    echo "Failed to retrieve Custom Resource Definition for WebLogic domain" >> domainAction.log
    echo "CRD: $CRD" >> domainAction.log
    exit 1
fi
...
echo "domain_api_version: $domain_api_version" >> domainAction.log
...
if [ $? -ne 0 ]
  then
    echo "Failed to retrieve WebLogic Domain Custom Resource Definition" >> domainAction.log
    echo "DOMAIN: $DOMAIN" >> domainAction.log
    exit 1
fi
echo "DOMAIN: $DOMAIN" >> domainAction.log
...
{
    "updateType": "introspect" 
}
EOF
)

echo "request_body: $request_body" >> domainAction.log

content_type="Content-Type: application/json"
requested_by="X-Requested-By: user"
...
then
  ...
  echo "Operator Cert File not found" >> domainAction.log
  exit 1
fi
...
echo "operator_url: $operator_url" >> domainAction.log
...
then
  ...
  echo "Operator PEM formatted file not found" >> domainAction.log
  exit 1
fi

if [ $? -ne 0 ]
then
  echo "Failed scaling request to WebLogic Operator" >> domainAction.log
  echo $result >> domainAction.log
  exit 1
fi
echo $result >> domainAction.log
