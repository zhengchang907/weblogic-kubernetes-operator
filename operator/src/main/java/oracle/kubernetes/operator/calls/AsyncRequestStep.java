...
public class AsyncRequestStep<T> extends Step implements RetryStrategyListener {
  ...
  class AsyncRequestStepProcessing {
    ...
    private CancellableCall createCall(Fiber fiber) throws ApiException {
    ...
    void onSuccess(Fiber fiber, T result, int statusCode, Map<String, List<String>> responseHeaders) {
      if (firstTimeResumed()) {
    ...
    void onFailure(Fiber fiber, ApiException ae, int statusCode, Map<String, List<String>> responseHeaders) {
      if (firstTimeResumed()) {
    ...
    private void handleTimeout(Fiber fiber, CancellableCall cc) {
      if (firstTimeResumed()) {
        try {
          cc.cancel();
        } finally {
    ...
    private void resumeAfterThrowable(Fiber fiber) {
      if (firstTimeResumed()) {
        addResponseComponent(Component.createFor(RetryStrategy.class, retryStrategy));
        fiber.resume(packet);
      }
  ...
  private void scheduleTimeoutCheck(Fiber fiber, int timeoutSeconds, Runnable timeoutCheck) {
    fiber.owner.getExecutor().schedule(timeoutCheck, timeoutSeconds, TimeUnit.SECONDS);
  }
  ...
  private class ApiCallbackImpl implements ApiCallback<T> {
    ...
    public ApiCallbackImpl(AsyncRequestStepProcessing processing, Fiber fiber) {
